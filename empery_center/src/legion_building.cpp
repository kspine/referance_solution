	...
	LegionBuildingUuid legion_building_uuid,   LegionUuid legion_uuid,  MapObjectUuid map_object_uuid,std::uint64_t ntype)
{
	...
	auto obj = boost::make_shared<MongoDb::Center_LegionBuilding>(legion_building_uuid.get(),legion_uuid.get() ,map_object_uuid.get(),ntype);
	obj->enable_auto_saving();
	auto promise = Poseidon::MongoDbDaemon::enqueue_for_saving(obj, false, true);
	auto account = boost::make_shared<LegionBuilding>(std::move(obj), std::vector<boost::shared_ptr<MongoDb::Center_LegionBuildingAttribute>>());
	return std::make_pair(std::move(promise), std::move(account));
}

LegionBuilding::LegionBuilding(boost::shared_ptr<MongoDb::Center_LegionBuilding> obj,
	const std::vector<boost::shared_ptr<MongoDb::Center_LegionBuildingAttribute>> &attributes)
	: m_obj(std::move(obj))
{
...
void LegionBuilding::set_attributes(boost::container::flat_map<LegionBuildingAttributeId, std::string> modifiers){
	...
	for(auto it = modifiers.begin(); it != modifiers.end(); ++it){
		...
		if(obj_it == m_attributes.end()){
			auto obj = boost::make_shared<MongoDb::Center_LegionBuildingAttribute>(m_obj->get_legion_building_uuid(),
