...
namespace EmperyCenter {
	...
	private:
		...
		boost::shared_ptr<MongoDb::Center_LegionDonate> m_stamps;

		boost::container::flat_map<AccountUuid,
			boost::shared_ptr<MongoDb::Center_LegionDonate>> m_donations;

	public:
		LegionDonateBox(LegionUuid legion_uuid,
			const std::vector<boost::shared_ptr<MongoDb::Center_LegionDonate>> &contributions);
		~LegionDonateBox();
