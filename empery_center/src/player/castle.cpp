	...
	for(auto it = map_cells.begin(); it != map_cells.end(); ++it){
		...
		if(amount_to_harvest != 0){
			...
			try {
	                ...
	                task_box->check(TaskBox::CAT_NULL,TaskTypeIds::ID_HARVEST_RESOURCES, 
								resource_id.get(), amount_harvested,castle, 0, 0);
...
PLAYER_SERVLET(Msg::CS_CastleHarvestSoldier, account, session, req){
	...
	try {
	        task_box->check(TaskBox::CAT_NULL,TaskTypeIds::ID_HARVEST_SOLDIERS, 
                                    info.map_object_type_id.get(), count_harvested,castle, 0, 0);

		const auto map_object_battalion = Data::MapObjectTypeBattalion::require(info.map_object_type_id);
		task_box->check(TaskBox::CAT_NULL,TaskTypeIds::ID_HARVEST_TYPE_SOLIDERS,
                                    map_object_battalion->map_object_weapon_id.get(), count_harvested,castle, 0, 0);
	} catch(std::exception &e){
		LOG_EMPERY_CENTER_WARNING("std::exception thrown: what = ", e.what());
	}
	return Response();
		...
		[&]{
			...
			if(member){
				...
				if(donate.empty()){
					...
					LegionLog::LegionPersonalDonateTrace(account->get_account_uuid(),boost::lexical_cast<uint64_t>(donate),
                                                          boost::lexical_cast<uint64_t>(donate) + amount_to_add,ReasonIds::ID_LEGION_USE_DONATE_ITEM,
                                                          item_id.get(),count_to_consume,0);	
				}
