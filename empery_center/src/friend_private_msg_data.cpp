...
FriendPrivateMsgData::FriendPrivateMsgData(FriendPrivateMsgUuid msg_uuid, LanguageId language_id, std::uint64_t created_time, AccountUuid from_account_uuid,std::vector<std::pair<ChatMessageSlotId, std::string>> segments)
	: m_obj(
		[&]{
			auto obj = boost::make_shared<MongoDb::Center_FriendPrivateMsgData>(msg_uuid.get(), language_id.get(), created_time, from_account_uuid.get(),encode_segments(segments));
...
FriendPrivateMsgData::FriendPrivateMsgData(boost::shared_ptr<MongoDb::Center_FriendPrivateMsgData> obj)
	: m_obj(std::move(obj))
