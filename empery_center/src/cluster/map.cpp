		...
		try {
			Poseidon::enqueue_async_job([=]() mutable {
               ...
               if(attacked_account_uuid){
					...
					task_box->check(TaskBox::CAT_NULL,task_type_id, TaskLegionPackageKeyIds::ID_DESTROY_SOLDIERS.get(), 
								attacked_type_data->warfare,castle_category, 0, 0);	
																												}
			   }
			   const auto monster_type_data = Data::MapObjectTypeMonster::get(attacked_object_type_id);
	           ...
	           if(monster_type_data){
			      ...
			      task_box->check(TaskBox::CAT_NULL,task_type_id, monster_type_data->level, 1,	castle_category, 0, 0);
			   }
		...
		try {
			Poseidon::enqueue_async_job([=]() mutable {
				...
				if (task_type_id == TaskTypeIds::ID_DESTROY_SOLDIERS)
				{
					...
					{
						task_box->check(TaskBox::CAT_LEGION_PACKAGE,task_type_id, TaskLegionPackageKeyIds::ID_DESTROY_SOLDIERS.get(), 
									soldiers_damaged*map_object_type_data->warfare,
							castle_category, 0, 0);
...
CLUSTER_SERVLET(Msg::KS_MapHarvestResourceCrate, cluster, req){
	...
	const auto amount_harvested = resource_crate->harvest(attacking_object, amount_to_harvest / unit_weight, forced_attack);
		...
		LOG_EMPERY_CENTER_DEBUG("Harvest: attacking_object_uuid = ", attacking_object_uuid, 
					",attacking_object_type_id = ", attacking_object_type_id,
					", amount_to_harvest = ", amount_to_harvest, 
					", amount_harvested = ", amount_harvested, 
					", forced_attack = ", forced_attack,
					" req.amount_harvested = ",req.amount_harvested,
