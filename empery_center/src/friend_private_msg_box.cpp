...
namespace {
	void fill_friend_private_msg_info(FriendPrivateMsgBox::PrivateMsgInfo &info, const boost::shared_ptr<MongoDb::Center_FriendPrivateMsgRecent> &obj){
		PROFILE_ME;
...
FriendPrivateMsgBox::FriendPrivateMsgBox(AccountUuid account_uuid,
	const std::vector<boost::shared_ptr<MongoDb::Center_FriendPrivateMsgRecent>> &recent_msgs)
	: m_account_uuid(account_uuid)
{
	...
	std::sort(m_recent_msgs.begin(), m_recent_msgs.end(),
		[](const boost::shared_ptr<MongoDb::Center_FriendPrivateMsgRecent> &lhs, const boost::shared_ptr<MongoDb::Center_FriendPrivateMsgRecent> &rhs){
...
void FriendPrivateMsgBox::push(std::uint64_t timestamp,AccountUuid friend_account_uuid,FriendPrivateMsgUuid msg_uuid,bool sender,bool read,bool deleted)
{
	...
	const auto obj = boost::make_shared<MongoDb::Center_FriendPrivateMsgRecent>(Poseidon::Uuid::random(),get_account_uuid().get(), timestamp,friend_account_uuid.get(),msg_uuid.get(),sender,read,deleted);
	obj->async_save(true);
