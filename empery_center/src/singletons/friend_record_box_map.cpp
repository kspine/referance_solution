...
namespace {
	struct FriendRecordBoxElement {
		...
		mutable boost::shared_ptr<std::vector<boost::shared_ptr<MongoDb::Center_FriendRecord>>> sink;
	...
	void gc_timer_proc(std::uint64_t now){
	    ...
	    Poseidon::MongoDbDaemon::enqueue_for_deleting("Center_FriendRecord", query, true);
	}
...
boost::shared_ptr<FriendRecordBox> FriendRecordBoxMap::get(AccountUuid account_uuid){
	...
	if(!it->friend_record_box){
		...
		do {
			if(!it->promise){
				auto sink = boost::make_shared<std::vector<boost::shared_ptr<MongoDb::Center_FriendRecord>>>();
			    Poseidon::MongoDb::BsonBuilder query;
				...
				auto promise = Poseidon::MongoDbDaemon::enqueue_for_batch_loading(
					[sink](const boost::shared_ptr<Poseidon::MongoDb::Connection> &conn){
						auto obj = boost::make_shared<MongoDb::Center_FriendRecord>();
						obj->fetch(conn);
						obj->enable_auto_saving();
						sink->emplace_back(std::move(obj));
					}, "Center_FriendRecord", std::move(query), 0, INT32_MAX);
