...
namespace {
	struct FriendPrivateMsgBoxElement {
		...
		mutable boost::shared_ptr<std::vector<boost::shared_ptr<MongoDb::Center_FriendPrivateMsgRecent>>> sink;
	...
	void gc_timer_proc(std::uint64_t now){
	    ...
	    Poseidon::MongoDbDaemon::enqueue_for_deleting("Center_FriendPrivateMsgRecent", query, true);
	}
	...
	struct FriendPrivateMsgDataElement {
		...
		mutable boost::shared_ptr<const Poseidon::JobPromise> promise;
		mutable boost::shared_ptr<boost::container::flat_map<LanguageId, boost::shared_ptr<MongoDb::Center_FriendPrivateMsgData>>> sink;

		mutable boost::shared_ptr<MongoDb::Center_FriendPrivateMsgData> msg_data_obj;
...
boost::shared_ptr<FriendPrivateMsgBox> FriendPrivateMsgBoxMap::get(AccountUuid account_uuid){
	...
	if(!it->friend_private_msg_box){
		...
		do {
			if(!it->promise){
				auto sink = boost::make_shared<std::vector<boost::shared_ptr<MongoDb::Center_FriendPrivateMsgRecent>>>();
				...
				auto promise = Poseidon::MongoDbDaemon::enqueue_for_batch_loading(
					[sink](const boost::shared_ptr<Poseidon::MongoDb::Connection> &conn){
						auto obj = boost::make_shared<MongoDb::Center_FriendPrivateMsgRecent>();
						obj->fetch(conn);
						obj->enable_auto_saving();
						sink->emplace_back(std::move(obj));
					}, "Center_FriendPrivateMsgRecent",  std::move(query), 0, INT32_MAX);
		...
		if(it->sink){
			...
			LOG_EMPERY_CENTER_DEBUG("Async MongoDb query completed: account_uuid = ", account_uuid, ", rows = ", it->sink->size());
...
boost::shared_ptr<FriendPrivateMsgData> FriendPrivateMsgBoxMap::get_msg_data(FriendPrivateMsgUuid msg_uuid, LanguageId language_id){
	...
	if(!it->msg_data){
		...
		if(!it->promise){
			auto sink = boost::make_shared<boost::container::flat_map<LanguageId, boost::shared_ptr<MongoDb::Center_FriendPrivateMsgData>>>();
            
            Poseidon::MongoDb::BsonBuilder query;
			query.append_uuid(sslit("msg_uuid"), msg_uuid.get());
			 
			auto promise = Poseidon::MongoDbDaemon::enqueue_for_batch_loading(
				[sink](const boost::shared_ptr<Poseidon::MongoDb::Connection> &conn){
					auto obj = boost::make_shared<MongoDb::Center_FriendPrivateMsgData>();
					obj->fetch(conn);
					obj->enable_auto_saving();
					const auto language_id = LanguageId(obj->get_language_id());
					sink->emplace(language_id, std::move(obj));
				}, "Center_FriendPrivateMsgData", std::move(query), 0, INT32_MAX);
			it->promise = std::move(promise);
			it->sink    = std::move(sink);
		}
		// 复制一个智能指针，并且导致 use_count() 增加。
		...
		if(it->sink){
			...
			LOG_EMPERY_CENTER_DEBUG("Async MongoDb query completed: msg_uuid = ", msg_uuid, ", rows = ", it->sink->size());

			boost::shared_ptr<MongoDb::Center_FriendPrivateMsgData> obj;
