...
#include "../mongodb/activity_config.hpp"
...
void  ActivityMap::force_load_activitys(Poseidon::JsonArray &activitys){
	...
	const auto sink = boost::make_shared<std::vector<boost::shared_ptr<MongoDb::Center_Activitys>>>();
	{
		const auto promise = Poseidon::MongoDbDaemon::enqueue_for_batch_loading(
				...
				auto obj = boost::make_shared<EmperyCenter::MongoDb::Center_Activitys>();
				obj->fetch(conn);
				obj->enable_auto_saving();
				sink->emplace_back(std::move(obj));
			}, "Center_Activitys", { }, 0, INT32_MAX);
    ...
    std::sort(sink->begin(), sink->end(),
    	[](const boost::shared_ptr<MongoDb::Center_Activitys>  &lhs, const boost::shared_ptr<MongoDb::Center_Activitys> &rhs)
          {
            return lhs->unlocked_get_activity_id() < rhs->unlocked_get_activity_id(); 
          }
    );
...
void  ActivityMap::force_load_activitys_map(Poseidon::JsonArray &activitys_map){
	...
	const auto sink = boost::make_shared<std::vector<boost::shared_ptr<MongoDb::Center_ActivitysMap>>>();
	{
		const auto promise = Poseidon::MongoDbDaemon::enqueue_for_batch_loading(
				[sink](const boost::shared_ptr<Poseidon::MongoDb::Connection> &conn) {
				auto obj = boost::make_shared<EmperyCenter::MongoDb::Center_ActivitysMap>();
				obj->fetch(conn);
				obj->enable_auto_saving();
				sink->emplace_back(std::move(obj));
			}, "Center_ActivitysMap", { }, 0, INT32_MAX);
		Poseidon::JobDispatcher::yield(promise, true);
	}
	if(sink->empty()){
    ...
    std::sort(sink->begin(), sink->end(),
    	[](const boost::shared_ptr<MongoDb::Center_ActivitysMap> &lhs, const boost::shared_ptr<MongoDb::Center_ActivitysMap> &rhs)
          {
             return lhs->unlocked_get_activity_id() < rhs->unlocked_get_activity_id(); 
          }
    );
...
void  ActivityMap::force_load_activitys_world(Poseidon::JsonArray &activitys_world){
	...
	const auto sink = boost::make_shared<std::vector<boost::shared_ptr<MongoDb::Center_ActivitysWorld>>>();
	{
		const auto promise = Poseidon::MongoDbDaemon::enqueue_for_batch_loading(
				[sink](const boost::shared_ptr<Poseidon::MongoDb::Connection> &conn) {
				auto obj = boost::make_shared<EmperyCenter::MongoDb::Center_ActivitysWorld>();
				obj->fetch(conn);
				obj->enable_auto_saving();
				sink->emplace_back(std::move(obj));
			}, "Center_ActivitysWorld", { }, 0, INT32_MAX);
		Poseidon::JobDispatcher::yield(promise, true);
	}
	...
	std::sort(sink->begin(), sink->end(),
    	[](const boost::shared_ptr<MongoDb::Center_ActivitysWorld> &lhs, const boost::shared_ptr<MongoDb::Center_ActivitysWorld> &rhs)
          {
             return lhs->unlocked_get_activity_id() < rhs->unlocked_get_activity_id(); 
          }
    );
...
void  ActivityMap::force_load_activitys_rank_award(Poseidon::JsonArray &rank_awards){
	...
	const auto sink = boost::make_shared<std::vector<boost::shared_ptr<MongoDb::Center_ActivitysRankAward>>>();
	{
		const auto promise = Poseidon::MongoDbDaemon::enqueue_for_batch_loading(
				[sink](const boost::shared_ptr<Poseidon::MongoDb::Connection> &conn) {
				auto obj = boost::make_shared<EmperyCenter::MongoDb::Center_ActivitysRankAward>();
				obj->fetch(conn);
				obj->enable_auto_saving();
				sink->emplace_back(std::move(obj));
			}, "Center_ActivitysRankAward", { }, 0, INT32_MAX);
		Poseidon::JobDispatcher::yield(promise, true);
	}
...
void  ActivityMap::force_update_activitys(std::uint64_t activity_id,std::string name,std::uint64_t begin_time,std::uint64_t end_time){
	...
	auto obj = boost::make_shared<MongoDb::Center_Activitys>(activity_id,name,begin_time,end_time);
	const auto promise = Poseidon::MongoDbDaemon::enqueue_for_saving(obj,true,true);
	Poseidon::JobDispatcher::yield(promise, true);
}
...
void  ActivityMap::force_update_activitys_map(std::uint64_t activity_id,std::string name,std::uint32_t type,std::uint32_t duration,std::string target){
	...
	auto obj = boost::make_shared<MongoDb::Center_ActivitysMap>(activity_id,name,type,duration,target);
	const auto promise = Poseidon::MongoDbDaemon::enqueue_for_saving(obj,true,true);
	Poseidon::JobDispatcher::yield(promise, true);
}
void  ActivityMap::force_update_activitys_world(std::uint64_t activity_id,std::string name,std::uint64_t pro_activity_id,std::uint64_t own_activity_id,std::uint32_t type,std::string target,std::string drop){
	...
	auto obj = boost::make_shared<MongoDb::Center_ActivitysWorld>(activity_id,name,pro_activity_id,own_activity_id,type,target,drop);
	const auto promise = Poseidon::MongoDbDaemon::enqueue_for_saving(obj,true,true);
	Poseidon::JobDispatcher::yield(promise, true);
}
void  ActivityMap::force_update_activitys_rank_award(std::uint64_t id,std::uint64_t activity_id,std::uint32_t rank_begin,std::uint32_t rank_end,std::string reward){
	...
	auto obj = boost::make_shared<MongoDb::Center_ActivitysRankAward>(id,activity_id,rank_begin,rank_end,reward);
	const auto promise = Poseidon::MongoDbDaemon::enqueue_for_saving(obj,true,true);
	Poseidon::JobDispatcher::yield(promise, true);
}
...
void  ActivityMap::force_delete_activitys(std::uint64_t activity_id){
	...
	del_query.append_unsigned(sslit("activity_id"),activity_id);
	const auto promise = Poseidon::MongoDbDaemon::enqueue_for_deleting("Center_Activitys", del_query, true);
	Poseidon::JobDispatcher::yield(promise, true);
}
void  ActivityMap::force_delete_activitys_map(std::uint64_t activity_id){
	...
	Poseidon::MongoDb::BsonBuilder del_query;
	del_query.append_unsigned(sslit("activity_id"),activity_id);
	const auto promise = Poseidon::MongoDbDaemon::enqueue_for_deleting("Center_ActivitysMap", del_query, true);
	Poseidon::JobDispatcher::yield(promise, true);
}
void  ActivityMap::force_delete_activitys_world(std::uint64_t activity_id){
	...
	Poseidon::MongoDb::BsonBuilder del_query;
	del_query.append_unsigned(sslit("activity_id"),activity_id);
	const auto promise = Poseidon::MongoDbDaemon::enqueue_for_deleting("Center_ActivitysWorld", del_query, true);
	Poseidon::JobDispatcher::yield(promise, true);
}
void  ActivityMap::force_delete_activitys_rank_award(std::uint64_t id){
	...
	Poseidon::MongoDb::BsonBuilder del_query;
	del_query.append_unsigned(sslit("id"),id);
	const auto promise = Poseidon::MongoDbDaemon::enqueue_for_deleting("Center_ActivitysRankAward", del_query, true);
	Poseidon::JobDispatcher::yield(promise, true);
}
