...
namespace {
	struct MapActivityAccumulateElement {
		boost::shared_ptr<MongoDb::Center_MapActivityAccumulate> obj;
		...
		MapActivityAccumulateElement(boost::shared_ptr<MongoDb::Center_MapActivityAccumulate> obj_)
	...
	struct MapActivityRankElement {
		boost::shared_ptr<MongoDb::Center_MapActivityRank> obj;
		std::pair<MapActivityId, std::uint64_t> activity_recent;
		std::pair<AccountUuid, std::pair<MapActivityId, std::uint64_t> > account_activity_recent;
		MapActivityRankElement(boost::shared_ptr<MongoDb::Center_MapActivityRank> obj_)
	...
	struct WorldActivityElement {
		boost::shared_ptr<MongoDb::Center_MapWorldActivity>                        obj;
		...
		WorldActivityElement(boost::shared_ptr<MongoDb::Center_MapWorldActivity> obj_)
	...
	struct WorldActivityAccumulateElement {
		boost::shared_ptr<MongoDb::Center_MapWorldActivityAccumulate>                       obj;
		std::pair<std::uint64_t,Coord>                                                    recent_cluster_coord;
		...
		WorldActivityAccumulateElement(boost::shared_ptr<MongoDb::Center_MapWorldActivityAccumulate> obj_)
			:obj(std::move(obj_)),recent_cluster_coord(std::make_pair(obj->get_since(),Coord(obj->get_cluster_x(),obj->get_cluster_y())))
	...
	struct WorldActivityRankElement {
		boost::shared_ptr<MongoDb::Center_MapWorldActivityRank>                    obj;
		std::pair<Coord,std::uint64_t>                                             cluster_recent;
		std::pair<AccountUuid,std::pair<Coord,std::uint64_t>>                      account_cluster_recent;
		WorldActivityRankElement(boost::shared_ptr<MongoDb::Center_MapWorldActivityRank> obj_)
	...
	struct WorldActivityBossElement {
		boost::shared_ptr<MongoDb::Center_MapWorldActivityBoss>                    obj;
		std::pair<Coord,std::uint64_t>                                             cluster_recent;
		WorldActivityBossElement(boost::shared_ptr<MongoDb::Center_MapWorldActivityBoss> obj_)
			:obj(std::move(obj_)),cluster_recent(Coord(obj->get_cluster_x(),obj->get_cluster_y()),obj->get_since())
		{
		}
	};
	...
	MODULE_RAII_PRIORITY(handles, 5000){
		...
		conn->execute_query("Center_MapActivityAccumulate", { }, 0, INT32_MAX);
		while(conn->fetch_next()){
			auto obj = boost::make_shared<MongoDb::Center_MapActivityAccumulate>();
			obj->fetch(conn);
		...
		conn->execute_query("Center_MapActivityRank", { }, 0, INT32_MAX);
		while(conn->fetch_next()){
			auto obj = boost::make_shared<MongoDb::Center_MapActivityRank>();
			obj->fetch(conn);
			obj->enable_auto_saving();
		...
		conn->execute_query("Center_MapWorldActivity", { }, 0, INT32_MAX);
		while(conn->fetch_next()){
			auto obj = boost::make_shared<MongoDb::Center_MapWorldActivity>();
			obj->fetch(conn);
			obj->enable_auto_saving();
		...
		conn->execute_query("Center_MapWorldActivityAccumulate", { }, 0, INT32_MAX);
		while(conn->fetch_next()){
			auto obj = boost::make_shared<MongoDb::Center_MapWorldActivityAccumulate>();
			obj->fetch(conn);
			obj->enable_auto_saving();
		...
		conn->execute_query("Center_MapWorldActivityRank", { }, 0, INT32_MAX);
		while(conn->fetch_next()){
			auto obj = boost::make_shared<MongoDb::Center_MapWorldActivityRank>();
			obj->fetch(conn);
			obj->enable_auto_saving();
		...
		conn->execute_query("Center_MapWorldActivityBoss", { }, 0, INT32_MAX);
		while(conn->fetch_next()){
			auto obj = boost::make_shared<MongoDb::Center_MapWorldActivityBoss>();
			obj->fetch(conn);
			obj->enable_auto_saving();
...
void MapActivityAccumulateMap::insert(AccumulateInfo info){
	...
	const auto obj = boost::make_shared<MongoDb::Center_MapActivityAccumulate>(info.account_uuid.get(), info.activity_id.get(),
...
void MapActivityRankMap::insert(MapActivityRankInfo info){
	...
	const auto obj = boost::make_shared<MongoDb::Center_MapActivityRank>(info.account_uuid.get(), info.activity_id.get(),
...
void WorldActivityMap::insert(WorldActivityInfo info){
	...
	const auto obj = boost::make_shared<MongoDb::Center_MapWorldActivity>(info.activity_id.get(), info.cluster_coord.x(),info.cluster_coord.y(),
...
void WorldActivityAccumulateMap::update(WorldActivityAccumulateInfo info){
	...
	if(it == world_activity_accumulate_map->end<0>()){
		const auto obj = boost::make_shared<MongoDb::Center_MapWorldActivityAccumulate>(info.account_uuid.get(),info.activity_id.get(),info.cluster_coord.x(),info.cluster_coord.y(),
...
void WorldActivityRankMap::insert(WorldActivityRankInfo info){
	...
	const auto obj = boost::make_shared<MongoDb::Center_MapWorldActivityRank>(info.account_uuid.get(),info.cluster_coord.x(),info.cluster_coord.y(),
...
void WorldActivityBossMap::update(WorldActivityBossInfo info){
	...
	if(it == world_activity_boss_map->end<0>()){
		const auto obj = boost::make_shared<MongoDb::Center_MapWorldActivityBoss>(info.cluster_coord.x(),info.cluster_coord.y(),info.since,info.boss_uuid.get(),info.create_date,info.delete_date,info.hp_total,info.hp_die);
		obj->async_save(true);
		world_activity_boss_map->insert(WorldActivityBossElement(std::move(obj)));
		return;
	}
