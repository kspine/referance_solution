...
namespace EmperyCenter{
  namespace{
		...
		{
			boost::shared_ptr<MongoDb::Center_NoviceGuide> novice_guide;
			AccountUuid account_uuid;
			TaskId task_id;
			std::uint64_t step_id;
		    explicit NoviceGuideElement(boost::shared_ptr<MongoDb::Center_NoviceGuide> novice_guide_)
		...
		MODULE_RAII_PRIORITY(handles, 5000)
		{
		  ...
		  {
		     boost::shared_ptr<MongoDb::Center_NoviceGuide> obj;
	      };
	      std::map<AccountUuid, TempNoviceGuideElement> temp_map;

		  LOG_EMPERY_CENTER_INFO("Loading Center_NoviceGuide...");

		  conn->execute_query("Center_NoviceGuide", { }, 0, INT32_MAX);

		  while (conn->fetch_next())
		  {
			 auto obj = boost::make_shared<MongoDb::Center_NoviceGuide>();
			 obj->fetch(conn);
	 ...
	 void NoviceGuideMap::insert(const boost::shared_ptr<MongoDb::Center_NoviceGuide> &novice_guide)
	 {
	 	PROFILE_ME;
	 ...
	 boost::shared_ptr<MongoDb::Center_NoviceGuide> NoviceGuideMap::find(AccountUuid account_uuid,TaskId task_id)
	 {
	       PROFILE_ME;
	 	const auto &novice_guide_map = g_novice_guide_map;
	 	if (!novice_guide_map)
