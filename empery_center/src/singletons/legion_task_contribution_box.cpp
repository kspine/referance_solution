...
namespace EmperyCenter {
	namespace {
		fill_task_contribution_info(LegionTaskContributionBox::TaskContributionInfo &info,boost::shared_ptr<MongoDb::Center_LegionTaskContribution> obj){
	...
	void LegionTaskContributionBox::insert(LegionTaskContributionBox::TaskContributionInfo info) {
		...
		const auto obj = boost::make_shared<MongoDb::Center_LegionTaskContribution>(get_legion_uuid().get(), account_uuid.get(),info.day_contribution,info.week_contribution,info.total_contribution,info.created_time);
		obj->async_save(true);
