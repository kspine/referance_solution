...
namespace {
	struct TaskBoxElement {
		...
		mutable boost::shared_ptr<std::vector<boost::shared_ptr<MongoDb::Center_Task>>> sink;
	...
	if(!it->task_box){
		...
		do {
			if(!it->promise){
				...
				oss <<"SELECT * FROM `Center_Task` WHERE `account_uuid` = " <<Poseidon::MongoDb::UuidFormatter(account_uuid.get());
