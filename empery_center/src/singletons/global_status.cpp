...
#include "../mongodb/global_status.hpp"

namespace EmperyCenter {

namespace {
	using GlobalStatusContainer = boost::container::flat_map<GlobalStatus::Slot, boost::shared_ptr<MongoDb::Center_GlobalStatus>>;
...
void GlobalStatus::reserve(GlobalStatus::Slot slot){
	...
	const auto it = g_global_status_map->find(slot);
	if(it == g_global_status_map->end()){
		auto obj = boost::make_shared<MongoDb::Center_GlobalStatus>((unsigned)slot, std::string());
		obj->async_save(true);
...
void GlobalStatus::set(GlobalStatus::Slot slot, std::string value){
	...
	const auto it = g_global_status_map->find(slot);
	if(it == g_global_status_map->end()){
		auto obj = boost::make_shared<MongoDb::Center_GlobalStatus>((unsigned)slot, std::move(value));
		obj->async_save(true);
		g_global_status_map->emplace(slot, std::move(obj));
	} else {
