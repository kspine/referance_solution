...
namespace {
	struct ItemBoxElement {
		...
		mutable boost::shared_ptr<std::vector<boost::shared_ptr<MongoDb::Center_Item>>> sink;
...
boost::shared_ptr<ItemBox> ItemBoxMap::get(AccountUuid account_uuid){
	...
	if(!it->item_box){
		...
		do {
			if(!it->promise){
				auto sink = boost::make_shared<std::vector<boost::shared_ptr<MongoDb::Center_Item>>>();
				std::ostringstream oss;
				oss <<"SELECT * FROM `Center_Item` WHERE `account_uuid` = " <<Poseidon::MongoDb::UuidFormatter(account_uuid.get());
				auto promise = Poseidon::MongoDbDaemon::enqueue_for_batch_loading(
					[sink](const boost::shared_ptr<Poseidon::MongoDb::Connection> &conn){
						auto obj = boost::make_shared<MongoDb::Center_Item>();
						obj->fetch(conn);
