...
boost::shared_ptr<MongoDb::Center_LegionFinancial> legionfinancials;
...
explicit LegionFinancialElement(boost::shared_ptr<MongoDb::Center_LegionFinancial> legionfinancials_)
...
MODULE_RAII_PRIORITY(handles,5000)
{
   ...
   {
      boost::shared_ptr<MongoDb::Center_LegionFinancial> obj;
   };
   std::map<LegionFinancialUuid,TempLegionFinancialElement> temp_map;
    
   conn->execute_query("Center_LegionFinancial", { }, 0, INT32_MAX);


   while(conn->fetch_next())
   {
      auto obj = boost::make_shared<MongoDb::Center_LegionFinancial>();
      obj->fetch(conn);
 ...
 void LegionFinancialMap::insert(const boost::shared_ptr<MongoDb::Center_LegionFinancial> &legionfinancials)
 {
   PROFILE_ME;
...
{
    auto obj = boost::make_shared<MongoDb::Center_LegionFinancial>((LegionFinancialUuid(Poseidon::Uuid::random())).get(),legion_uuid.get(),account_uuid.get(),item_id.get(), old_count, new_count, delta_count,action_id,action_count, created_time);
    obj->enable_auto_saving();
 ...
 void  LegionFinancialMap::get_legion_financial(LegionUuid legion_uuid,std::vector<boost::shared_ptr<MongoDb::Center_LegionFinancial>> &ret)
 {
   PROFILE_ME;
   const auto &legion_financial_map = g_legion_financial_map;
   if(!legion_financial_map){
