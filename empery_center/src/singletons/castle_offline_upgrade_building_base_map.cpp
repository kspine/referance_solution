...
namespace EmperyCenter{
   ...
   namespace {
       ...
       struct CastleOfflineUpgradeBuildingBaseElement{
          ...
          boost::shared_ptr<MongoDb::Center_CastleOfflineUpgradeBuildingBase> castles;
          std::string auto_uuid;
          AccountUuid account_uuid;
          MapObjectUuid map_object_uuid;
          std::string building_base_ids;

          explicit CastleOfflineUpgradeBuildingBaseElement(boost::shared_ptr<MongoDb::Center_CastleOfflineUpgradeBuildingBase> castles_)
      ...
      const auto conn = Poseidon::MongoDbDaemon::create_connection();
         ...
         struct TempCastlesElement {
          boost::shared_ptr<MongoDb::Center_CastleOfflineUpgradeBuildingBase> obj;
         };

         std::map<std::string,TempCastlesElement> temp_castles_map;

         conn->execute_query("Center_CastleOfflineUpgradeBuildingBase", { }, 0, INT32_MAX);

         while(conn->fetch_next()){
             auto obj = boost::make_shared<MongoDb::Center_CastleOfflineUpgradeBuildingBase>();
             obj->fetch(conn);
  ...
  void CastleOfflineUpgradeBuildingBaseMap::insert(const boost::shared_ptr<MongoDb::Center_CastleOfflineUpgradeBuildingBase> &obj)
  {
      PROFILE_ME;
  ...
  void CastleOfflineUpgradeBuildingBaseMap::make_insert(MapObjectUuid map_object_uuid,std::uint64_t building_base_id)
  {
	 ...
	 if(!map_object)
	 {
      ...
      auto obj = boost::make_shared<MongoDb::Center_CastleOfflineUpgradeBuildingBase>(str_auto_uuid,(map_object->get_owner_uuid()).get(),map_object_uuid.get(),building_base_ids); 
      obj->enable_auto_saving();
   ...
   boost::shared_ptr<MongoDb::Center_CastleOfflineUpgradeBuildingBase> CastleOfflineUpgradeBuildingBaseMap::find(AccountUuid account_uuid,MapObjectUuid map_object_uuid)
   {
      PROFILE_ME;
      const auto &castles_map = g_castles_map;
      if(!castles_map){
   ...
   void CastleOfflineUpgradeBuildingBaseMap::deleted(AccountUuid account_uuid,MapObjectUuid map_object_uuid)
   {
      ...
      query.append_string(sslit("_id"), str_auto_uuid);
      Poseidon::MongoDbDaemon::enqueue_for_deleting("Center_CastleOfflineUpgradeBuildingBase", query, true);
   }
