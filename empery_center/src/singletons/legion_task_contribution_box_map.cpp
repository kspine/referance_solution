...
namespace {
	struct LegionTaskContributionBoxElement {
		...
		mutable boost::shared_ptr<std::vector<boost::shared_ptr<MongoDb::Center_LegionTaskContribution>>> sink;
...
boost::shared_ptr<LegionTaskContributionBox> LegionTaskContributionBoxMap::get(LegionUuid legion_uuid){
	...
	if(!it->legion_task_contribution_box){
		...
		do {
			if(!it->promise){
				auto sink = boost::make_shared<std::vector<boost::shared_ptr<MongoDb::Center_LegionTaskContribution>>>();
				std::ostringstream oss;
				oss <<"SELECT * FROM `Center_LegionTaskContribution` WHERE `legion_uuid` = " <<Poseidon::MongoDb::UuidFormatter(legion_uuid.get());
				auto promise = Poseidon::MongoDbDaemon::enqueue_for_batch_loading(
					[sink](const boost::shared_ptr<Poseidon::MongoDb::Connection> &conn){
						auto obj = boost::make_shared<MongoDb::Center_LegionTaskContribution>();
						obj->fetch(conn);
