...
namespace EmperyCenter {
	namespace {
		void fill_donate_info(LegionDonateBox::DonateInfo &info,boost::shared_ptr<MongoDb::Center_LegionDonate> obj){
	...
	LegionDonateBox::LegionDonateBox(LegionUuid legion_uuid,
		const std::vector<boost::shared_ptr<MongoDb::Center_LegionDonate>> & donations)
	...
	void LegionDonateBox::update(AccountUuid account_uuid,std::uint64_t delta) {
		...
		const auto it = m_donations.find(account_uuid);
		if (it == m_donations.end()) {
			const auto obj = boost::make_shared<MongoDb::Center_LegionDonate>(get_legion_uuid().get(), account_uuid.get(),delta,delta,delta,utc_now);
			obj->async_save(true);
			m_donations.emplace(account_uuid,obj);
			return;
		}
