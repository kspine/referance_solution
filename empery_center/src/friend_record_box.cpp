...
namespace {
	void fill_record_info(FriendRecordBox::RecordInfo &info, const boost::shared_ptr<MongoDb::Center_FriendRecord> &obj){
		PROFILE_ME;
...
FriendRecordBox::FriendRecordBox(AccountUuid account_uuid,
	const std::vector<boost::shared_ptr<MongoDb::Center_FriendRecord>> &records)
	: m_account_uuid(account_uuid)
{
	...
	std::sort(m_records.begin(), m_records.end(),
		[](const boost::shared_ptr<MongoDb::Center_FriendRecord> &lhs, const boost::shared_ptr<MongoDb::Center_FriendRecord> &rhs){
...
void FriendRecordBox::push(std::uint64_t timestamp,AccountUuid friend_account_uuid,int result_type)
{
	...
	const auto obj = boost::make_shared<MongoDb::Center_FriendRecord>(Poseidon::Uuid::random(),get_account_uuid().get(), timestamp,friend_account_uuid.get(),result_type,0);
	obj->async_save(true);
